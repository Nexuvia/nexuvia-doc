...
| 13 | Wire `@nexuvia/cart` — `/api/cart` routes + provider; use `@nexuvia/cart/server` in routes, `@nexuvia/cart/client` in providers | [cart](/wiring/cart) |
...
| **Use `@nexuvia/cart/server` in route handlers, `@nexuvia/cart/client` in providers** | OCC adapter blocked by CORS in browser |
